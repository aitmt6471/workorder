...
/* ══════════════════════════════════════════════════════════════
   아이템 복사(클론): 기존 아이템의 모든 문서를 새 아이템으로 딥카피
   - 서버(n8n ait/cars/copy)에서 단일 트랜잭션으로 전부 복사
     (CP·작표·설비일상·초중종물·마스터샘플·개정이력/서명·사진링크·
      공정검사기준서·Q-Point·알람규칙). 실적(점검결과)만 제외.
   - 복사본은 새 행으로 INSERT → 원본과 완전 독립
   ══════════════════════════════════════════════════════════════ */
async function cloneCar(srcId) {
  ...
  const newName = (prompt(`"${srcName}" 의 모든 문서(CP·작표·설비일상·초중종물·마스터샘플·개정이력 등)를 복사합니다.\n실적(점검결과)은 복사되지 않습니다.\n\n새 아이템 이름:`, srcName + ' (복사)') || '').trim();
  if (!newName) return;
  if (cars.some(c => c.name === newName)) { alert('같은 이름의 아이템이 이미 있습니다.'); return; }

  const toast = (m, t) => window.showToast && window.showToast(m, t || 'info');
  toast('복사 중… (문서량에 따라 최대 1분 소요)');

  try {
    const res = await AIT_API.copyCar({ sourceCarId: srcId, name: newName });
    const r = Array.isArray(res) ? res[0] : res;
    if (!r || r.ok === false || !r.id) throw new Error((r && r.error) || '서버 복사 실패');
    toast('복사 완료 ✓', 'success');
    await initCars();
    renderCarListInModal();
  } catch (e) {
