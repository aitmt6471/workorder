...
window.print_cp_gentex = async function() {
  ...
  // ── DB 페치 ───────────────────────────────────────────────────────────────
  // 개정이력·결재서명도 선택 탭(차종)별로 조회: 공통='cp', 차종='cp@차종'
  const _revDt = (typeof _revGroupKey === 'function') ? _revGroupKey('cp')
               : ((window._cpActiveVariant && window._cpActiveVariant !== '공통') ? 'cp@' + window._cpActiveVariant : 'cp');
  const [revisionsRaw, signs, cpRowsRaw, cpMetaDb] = await Promise.all([
    AIT_API.getRevisions(carId, _revDt).catch(() => []),
    AIT_API.getRevisionSigns(carId, _revDt).catch(() => []),
    AIT_API.getCpRows(carId).catch(() => []),
    Promise.resolve(null)   // ait/cp/meta 미사용(레거시 죽은 엔드포인트): CORS/500·signal 콘솔에러 방지. cft는 로컬캐시 폴백
  ]);
  // 작표·설비일상 전용 서브개정(rev_display "9-1" 형태)은 CP 인쇄본에 나타나면 안 됨
  const revisions = (revisionsRaw || []).filter(r => !/^\d+-\d+$/.test(String(r.rev_display || '').trim()));
...
<style>
@page :first { size: A4 portrait;  margin: 10mm; }
